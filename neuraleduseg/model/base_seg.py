...
class BaseSegModel:
    def __init__(self, args):
        ...
        self.logger.info('Trainable variables {}'.format(self.all_params))
        ...
        if self.use_ema:
            ...
            with tf.compat.v1.variable_scope('backup_variables'):
                self.bck_vars = [
                    tf.compat.v1.get_variable(var.op.name, dtype=var.value().dtype, trainable=False,
                                              initializer=var.initialized_value()) for var in self.all_params
                ]
            self.ema_backup_op = tf.group(*(tf.compat.v1.assign(bck, var.read_value())
                                            for bck, var in zip(self.bck_vars, self.all_params)))
            self.ema_restore_op = tf.group(*(tf.compat.v1.assign(var, bck.read_value())
                                             for bck, var in zip(self.bck_vars, self.all_params)))
            self.ema_assign_op = tf.group(*(tf.compat.v1.assign(var, self.ema.average(var).read_value())
        ...
        self.saver = tf.compat.v1.train.Saver(self.all_params)
    ...
    def train(self, dataset, epochs, batch_size, print_every_n_batch=0):
        ...
        for epoch in range(1, epochs + 1):
            ...
            dev_perf = self.evaluate(dev_batches, print_every_n_batch, print_result=True)
